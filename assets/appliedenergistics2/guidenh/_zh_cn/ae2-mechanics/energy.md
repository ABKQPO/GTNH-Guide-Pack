...
出于显而易见的原因，AE网络在一个游戏刻内能够输入或消耗的能量不能超过它能存储的总能量上限。如果一个网络的能量缓存上限是800AE，那当它的[设备](../ae2-mechanics/devices.md)请求能量时，它们最多只能使用 800AE（假设存储已满），且能量接收器1tick只能向网络中输入最多800AE（假设存储为空）。这个设定在使用[空间IO](./spatial-io.md)时体现得尤为明显，你需要准备很多能源元件才能启动[空间塔](../items-blocks/spatial_pylon.md)。
